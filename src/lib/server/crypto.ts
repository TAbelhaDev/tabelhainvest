...
//     offline brute force is cheap.
//
//   * Nothing binds a ciphertext to what it was for, so a row copied into
//     another purpose or user decrypts only under the same context. The context
//     string feeds HKDF *and* is passed as AES-GCM additional data.
//
//   * The payload carries a version (`v`), so rotating the scheme never makes
//     stored secrets unreadable with no way to tell why.
...
function contextBytes(context: SecretContext): Uint8Array<ArrayBuffer> {
	return toBytes(`tabelainvest:v${CURRENT_VERSION}:${context.purpose}:${context.userId}`);
}
	...
	return crypto.subtle.deriveKey(
		{
			...
			salt: toBytes('tabelainvest/credentials'),
